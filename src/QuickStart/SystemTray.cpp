...
LRESULT CSystemTray::OnTrayNotification(UINT wParam, LONG lParam) 
{
    ...
    if (LOWORD(lParam) == WM_RBUTTONDOWN || LOWORD(lParam) == WM_LBUTTONDOWN)
    {    
        if (!menu.LoadMenu(m_tnd.uID)) return 0;
        if (!(pSubMenu = menu.GetSubMenu(0))) return 0;

        // let parent fill the popup menu
		pTarget->SendMessage(WM_FILLSYSTRAYMENU, 0, (LPARAM) pSubMenu->GetSafeHmenu() );
