...
CButtonST::CButtonST()
{
	...
	m_pszWndText		= 0;
	m_nTextFormat		= DT_WORDBREAK | DT_CENTER;
...
void CButtonST::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	if (m_pszWndText && m_pszWndText->GetLength())
		sTitle = *m_pszWndText;
	else
	...
	if (sTitle.IsEmpty() == FALSE)
	{
		...
		*/
		...
		pDC->DrawText(sTitle, -1, captionRect, m_nTextFormat | DT_CALCRECT);
		...
		*/
		if (m_bIsDisabled)
		{
			...
			pDC->DrawText(sTitle, -1, captionRect, m_nTextFormat);
			captionRect.OffsetRect(-1, -1);
...
void CButtonST::PrepareImageRect(BOOL bHasTitle, RECT* rpItem, CRect* rpTitle, BOOL bIsPressed, DWORD dwWidth, DWORD dwHeight, CRect* rpImage)
{
	...
	switch (m_nAlign)
	{
		...
		case ST_ALIGN_VERT:
			...
			{
				rpImage->top += 3; // ADJUSTED BY SOFT-CENTRAL
								   // WAS rpImage->top = 3;
...
void CButtonST::SetDrawTextFormat(UINT nFormat)
{
